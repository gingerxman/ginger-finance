...
DELIMITER $$
CREATE FUNCTION record_trigger_action(vname VARCHAR(1024), vcode VARCHAR(125), vrowid INT, vstatus VARCHAR(128), vmsg VARCHAR(1024)) RETURNS TINYINT
BEGIN
    /*触发器日志*/
    INSERT INTO system_trigger_log(`name`, `code`, `row_id`, `status`, `msg`, `created_at`) values(vname, vcode, vrowid, vstatus, vmsg, NOW());
    RETURN 1;
END
$$
DELIMITER ;
...
BEGIN
    /*账户余额不需要更新*/
    return LOCATE(".0", code) || LOCATE("user_rmb.", code);
END
$$
DELIMITER ;
...
BEGIN
    /*账户可透支*/
    return LOCATE(".0", code) || is_debtable;
END
$$
DELIMITER ;
...
BEGIN
    ...
    DECLARE trigger_code VARCHAR(125);
    ...
    SET trigger_code = concat('transfer-', new.bid);
    SET fn_result = record_trigger_action(trigger_name, trigger_code, new.id, "triggered", '');
    
    IF new.source_amount < 0 || new.dest_amount < 0 THEN 
        SET fn_result = record_trigger_action(trigger_name, trigger_code, new.id, "failed", "交易金额不能为负");
    ...
    IF !should_ignore_balance(source_account_code) THEN
        ...
        IF ROW_COUNT() <> 1 THEN
            SET fn_result = record_trigger_action(trigger_name, trigger_code, new.id, "failed", "账户余额不足");
            SIGNAL SQLSTATE 'HY000' SET MESSAGE_TEXT = "[mysql trigger]: 账户余额不足";
        END IF;
        SET fn_result = record_trigger_action(trigger_name, trigger_code, new.id, "done", "minus_source");
    END IF;
    ...
    IF !should_ignore_balance(dest_account_code) THEN
        ...
        SET fn_result = record_trigger_action(trigger_name, trigger_code, new.id, "done", "plus_dest");
    END IF;
END
$$
DELIMITER ;
...
BEGIN
    ...
    SET trigger_code = concat("record-", new.id);
    SET fn_result = record_trigger_action(trigger_name, trigger_code, new.id, "triggered", "");
    ...
    IF (account_frozen_amount + new.amount > account_balance) && !can_overdraw_account(account_code, account_is_debtable) THEN
        SET fn_result = record_trigger_action(trigger_name, trigger_code, new.id, "failed", "余额不足,无法完成资金冻结");
    ...
    END IF;
    SET fn_result = record_trigger_action(trigger_name, trigger_code, new.id, "done", "succeed");
END
$$
DELIMITER ;
    ...
    IF old.status<>new.status THEN
        IF new.status = 2 || new.status = 3 THEN
            ...
            IF account_frozen_amount-new.amount < 0 THEN
                SET fn_result = record_trigger_action(trigger_name, trigger_code, new.id, "failed", "冻结金额已成负值");
            ...
            ELSE
                ...
                IF ROW_COUNT() <> 1 THEN
                    SET fn_result = record_trigger_action(trigger_name, trigger_code, new.id, "failed", "账户冻结金额不足");
